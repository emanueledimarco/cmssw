  ...
  EcalUncalibRecHitWorkerBaseClass( ps )
{
  ...
  shapesFileName_ = ps.getUntrackedParameter<std::string>("ShapesFileName");

  savePlot_ = ps.getUntrackedParameter<bool>("SavePlot"); 
  algoEB_.SavePlot(savePlot_);
  algoEE_.SavePlot(savePlot_);
}
...
EcalUncalibRecHitWorkerAnalFit::set(const edm::EventSetup& es)
{
  ...
  // fetch the shapes of the in time and out of time from a ROOT file (temporary)
  TFile *fileTemplates = TFile::Open(shapesFileName_.c_str());

  for( int s=0; s<2; ++s) {
    const std::string namesector ( 0 == s ? "Barrel" : "Endcap") ;
    const std::string nameh  ( "Ecal" + namesector + "Shape" ) ;
    TH1D *shape = (TH1D*)fileTemplates->Get(nameh.c_str());
    
    // Get the OOT PU shapes (shifted replicas of the IT one)
    std::vector<TH1D*> oot_shapes;
    // replicate it for -10bx +3bx
    for (int i = -10; i<4; ++i) {
      if(i==0) continue;
      char buf[100];
      if(i>0) sprintf(buf,"%s_oot+%dbx",nameh.c_str(),i);
      else sprintf(buf,"%s_oot%dbx",nameh.c_str(),i);
      TH1D *ootshape = (TH1D*)fileTemplates->Get(buf);
      oot_shapes.push_back(ootshape);
    }
    if(s == 0) {
      algoEB_.SetInTimeShape(shape);
      algoEB_.SetOutOfTimeShapes(oot_shapes);
    } else {
      algoEE_.SetInTimeShape(shape);
      algoEE_.SetOutOfTimeShapes(oot_shapes);
    }
  }
