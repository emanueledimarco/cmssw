...
CmsEcalRecHitFiller::~CmsEcalRecHitFiller() 
{
  ...
  delete privateData_->ootenergy;
  delete privateData_->ootchi2;
}
...
void CmsEcalRecHitFiller::writeEcalRecHitInfo(const EcalRecHit *cand, 
  ...
  privateData_->ootenergy ->push_back( cand->outOfTimeEnergy() );
  privateData_->ootchi2   ->push_back( cand->outOfTimeChi2() );
...
void CmsEcalRecHitFiller::treeEcalRecHitInfo(const std::string colPrefix, const std::string colSuffix) 
{
  ...
  cmstree->column((colPrefix+"ootenergy"+colSuffix).c_str(), *privateData_->ootenergy, nCandString.c_str(), 0, "Reco");
  cmstree->column((colPrefix+"ootchi2"+colSuffix).c_str(), *privateData_->ootchi2, nCandString.c_str(), 0, "Reco");
}
...
void CmsEcalRecHitFillerData::initialise() 
{
  ...
  r9 = new vector<float>;
  ootenergy = new vector<float>;
  ootchi2 = new vector<float>;
}
