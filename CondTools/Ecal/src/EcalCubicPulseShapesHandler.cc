...
template <class P>
popcon::EcalCubicPulseShapesHandler<P>::EcalCubicPulseShapesHandler(const edm::ParameterSet& ps)
...
template <class P>
popcon::EcalCubicPulseShapesHandler<P>::~EcalCubicPulseShapesHandler() {}

template <class P>
bool popcon::EcalCubicPulseShapesHandler<P>::checkPulseShape(EcalCubicPulseShapeT<P>::Item* item) {
  // true means all is standard and OK
  bool result = true;
  for (int s = 0; s < EcalCubicPulseShapeT<P>::TEMPLATESAMPLES; ++s) {
    if (s % EcalCubicPulseShapeT<P>::PARSPERSAMPLE == 0 && (item->parameters[s] > 1 || item->parameters[s] < 0))
      result = false;
    if (s % EcalCubicPulseShapeT<P>::PARSPERSAMPLE != 0 && (fabs(item->parameters[s]) > 1))
      result = false;
  }
  return result;
}
...
template <class P>
void popcon::EcalCubicPulseShapesHandler<P>::fillSimPulseShape(EcalCubicPulseShapeT<P>::Item* item, bool isbarrel) {
  for (int s = 0; s < EcalCubicPulseShapeT<P>::TEMPLATESAMPLES; ++s) {
...
template <class P>
void popcon::EcalCubicPulseShapesHandler<P>::getNewObjects() {
  std::cout << "------- Ecal - > getNewObjects\n";

  // create the object pukse shapes
  EcalCubicPulseShapeT<P>* pulseshapes = new EcalCubicPulseShapeT<P>();

  // read the templates from a text file
  std::ifstream inputfile;
  inputfile.open(m_filename.c_str());
  float templatevals[EcalCubicPulseShapeT<P>::TEMPLATESAMPLES];
  unsigned int rawId;
  int isbarrel;
  std::string line;
  ...
  if (m_firstRun > 1) {
    while (std::getline(inputfile, line)) {
      ...
      typename EcalCubicPulseShapeT<P>::Item item;
      for (int s = 0; s < EcalCubicPulseShapeT<P>::TEMPLATESAMPLES; ++s)
  ...
  this->m_to_transfer.push_back(std::make_pair((EcalCubicPulseShapeT<P>*)pulseshapes, snc));
