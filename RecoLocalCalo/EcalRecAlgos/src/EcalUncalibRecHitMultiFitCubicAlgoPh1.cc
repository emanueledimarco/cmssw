#include "RecoLocalCalo/EcalRecAlgos/interface/EcalUncalibRecHitMultiFitCubicAlgoPh1.h"
...
EcalUncalibRecHitMultiFitCubicAlgoPh1::EcalUncalibRecHitMultiFitCubicAlgoPh1()
    : _computeErrors(true),
      _doPrefit(false),
      _prefitMaxChiSq(1.0),
...
/// compute rechits
EcalUncalibratedRecHit EcalUncalibRecHitMultiFitCubicAlgoPh1::makeRecHit(const EcalDataFrame &dataFrame,
  ...
  const unsigned int iSampleMax = ecalPh1::kMaxSampleIdx;
  const unsigned int iFullPulseMax = ecalPh1::kMaxSampleIdx - ecalPh1::kMinBX - 1;
  ...
  if (!usePrefit) {
    ...
    if (!status) {
      edm::LogWarning("EcalUncalibRecHitMultiFitCubicAlgoPh1::makeRecHit") << "Failed Fit" << std::endl;
    }
