...
process.globalEvent = cms.Sequence( process.goodPrimaryVertices )
# this is needed only for 42X, since after that the isolation is corrected with the kt6PFJets rho
if(is42X):
    process.globalEvent *= process.FastjetForIsolation # for sync'ed regression
    process.globalEvent *= process.kt6PFJets # for PFMet corrections
    process.eleRegressionEnergy.rhoCollection = cms.InputTag("kt6PFJetsForIsolation","rho")
else:
    if(isFastSim):
