...
void CmsEleIDTreeFiller::writeCollectionToTree(edm::InputTag collectionTag,
  ...
  if(collection) {
    ...
    if(tracksProducer_.label().size()!=0) {
...
void CmsEleIDTreeFiller::writeEleInfo(const GsfElectronRef electronRef,
  ...
  if(tracksProducer_.label().size()!=0) {
    ...
    privateData_->mvaidnontrig->push_back(-999.);
    privateData_->mvaidtrig->push_back(-999.);
  }
  ...
  } else privateData_->mvaidisotrig->push_back(-999.);
