...
private:
  ...
  class LPmap {
  private:
    vector<CmsTreeColumn*> map;
  public:
    LPmap(){}
    ~LPmap(){ map.clear(); }
    void append( CmsTreeColumn* colp ) { map.push_back( colp ); }
    CmsTreeColumn* getColumn( const char* ) const;
    const vector<CmsTreeColumn*> & getMap() const { return map; }
  };

  // Data members of CmsTree:
  LPmap lpmap;
  TTree* treep;
